  ...
  text.split("\n\n").forEach((block) => {
    ...
    while (true) {
      ...
      if (useComment) {
        ...
        // determine background style class for common colors ()
...
export default function AnnotatedTranscript({
  ...
  console.log('[AnnotatedTranscript] Received segments:', segments?.length || 0);
  ...
  const md = useMemo(() => {
    if (segments && segments.length > 0) {
      const markdown = segments
        .map((s) => `*${s.title}*\n\n${s.content}`)
        .join("\n\n");
      console.log('[AnnotatedTranscript] Generated markdown length:', markdown.length);
      return markdown;
    }
    console.warn('[AnnotatedTranscript] No segments provided');
    return "";
  }, [segments]);

  const blocks = useMemo(() => {
    const result = renderMarkdownBlocks(md);
    console.log('[AnnotatedTranscript] Rendered blocks:', result.length);
    return result;
  }, [md]);
  ...
  if (!segments || segments.length === 0) {
    return (
      <div className="bg-white p-4 rounded-md border border-gray-200">
        <p className="text-gray-500 text-center">No annotated transcript available.</p>
      </div>
    );
  }
  ...
  if (blocks.length === 0) {
    return (
      <div className="bg-white p-4 rounded-md border border-gray-200">
        <p className="text-gray-500 text-center">Unable to parse transcript segments.</p>
        <details className="mt-2">
          <summary className="cursor-pointer text-sm text-gray-400">Debug info</summary>
          <pre className="mt-2 text-xs overflow-auto">{JSON.stringify(segments, null, 2)}</pre>
        </details>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white p-0 pl-5 rounded-md overflow-y-auto max-h-full">
      <div>
        {blocks.map((block, i) => (
          <div
            key={i}
          ...
          >
            ...
            <div className="md:w-45 py-1">
              {block.comment && (
                <div className="bg-orange-50 border-l-4 border-orange-400 rounded px-2 py-1 text-gray-800 break-words">
                  {block.comment}
                </div>
              )}
            </div>
